...
/*
* created TwoSum method that returns the indices of two elements in the nums array that add up to the target value
* looped through each element in the nums array
* inside the loop, initialized current using nums[i]
* still inside the loop, calculated the complement by subtracting target from current 
* if statement to check if complement is in the HashMap, if found, returned the index of the complement and current 
* if not found, put the current element and it's index in the HashMap
* throw an IllegalArgumentException if no two sum solution is found
